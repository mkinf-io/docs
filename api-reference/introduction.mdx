...
```
### Email and password
If you want to authenticate your requests using your email and password use the [Sign in](apidog://link/endpoint/9702878) endpoint.
```http
...
```
If you want to refresh your Access Token use the [same endpoint](apidog://link/endpoint/9702878) as follow
```http
POST /v0.1/auth/token HTTP/1.1
Host: api.mkinf.io
Content-Type: application/json
...
## List organizations
Retrieve all your organizations using the [Lists Organizations](apidog://link/endpoint/9770469) endpoint
```http
...
## List projects
Retrieve all your organization project using the [Lists Projects](apidog://link/endpoint/9680952) endpoint
```http
...
You can access your billing dashboard using [Billing](api-9650566).
```http
...
Check for available GPUs using the [availability](api-9650537) and [specs](api-9650538) endpoints.

### [List availabilities](api-9650537)
...
## Launch a VM
You can [create a new VM](api-9650534) using the `type` field to specify the model and amount of GPU. You need also to pass the `location` where you want to launch the VM, you can find all the available locations using the [availability](api-9650537) or [locations](api-9650564) endpoints. Finally you have to pass your `ssh_public_key`.
...
### 2. [List available OS images](api-9650563)
...
```
### 4. [Check the VM operation status](api-9650540)
...
When you don't need an instance anymore you can [terminate](api-9650536) it.

## Firewall Rules
Setup your [VPC Firewall Rules](api-9650552) to have desidered access to VMs.
...
```
Use [List Networks](api-9650558) to retrieve your `vpc_network_id` and [List VMs](api-9650533) to retrieve your VM `id` and set it as `resource_id`.

## Usage
You can use [Usage](api-9650565) to retreives the products usage. Usage is updated daily shortly after midnight UTC.
